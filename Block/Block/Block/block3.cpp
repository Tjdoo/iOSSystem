...
/**
 *  @brief   __Block_byref_a_0 是一个结构体，并且有一个 isa，因此可以知道它其实就是一个对象。
  */
...
struct __main_block_impl_0 {
  ...
  __Block_byref_a_0 *a; // by ref   捕获的变量，这里是内部创建的结构体指针
...
static void __main_block_func_0(struct __main_block_impl_0 *__cself) {
  __Block_byref_a_0 *a = __cself->a; // bound by ref   // copy 结构体的指针

        (a->__forwarding->a)++;   // a->__forwarding->a 和 oc 代码中的 a 指向的是同一片内存空间，所以能够执行 a++，注意这里的 a 是在堆上
        printf("Hello world --- %d\n", (a->__forwarding->a));
    }

/**
  *  @brief   __main_block_copy_0 中调用的是 _Block_object_assign。__block 变量结构体实例从栈拷贝到堆时调用
  */
static void __main_block_copy_0(struct __main_block_impl_0*dst, struct __main_block_impl_0*src) {_Block_object_assign((void*)&dst->a, (void*)src->a, 8/*BLOCK_FIELD_IS_BYREF*/);}

/**
  *  @brief    __main_block_dispose_0 中调用的是 _Block_object_dispose。__block 变量结构体实例引用计数为 0 时调用
  */
...
//    __attribute__((__blocks__(byref))) __Block_byref_a_0 a = {(void*)0,(__Block_byref_a_0 *)&a, 0, sizeof(__Block_byref_a_0), 10};
    ...
    // 精简（去除强转）：
    // &a 是上面的 __Block_byref_a_0 类型结构体指针。block 捕获的是 __Block_byref_a_0 类型指针
