...
struct __main_block_impl_0 {
  ...
  int a; // 成员变量。block 自动捕获外部变量，保存到了 block 的结构体实例里面
    
  /*
        参数 1：__main_block_func_0 函数中其实存储着我们在 block 中写下的代码。而__main_block_impl_0 函数中传入的是(void *)__main_block_func_0，也就说将我们写在 block 块中的代码封装成 __main_block_func_0 函数，并将 __main_block_func_0 函数的地址传入了 __main_block_impl_0 的构造函数中保存在结构体内。
   
        参数 2：__main_block_desc_0 中存储着两个参数：reserved 和 Block_size，最终将 __main_block_desc_0 结构体的地址传入 __main_block_func_0 中赋值给 Desc
   
        参数 3：我们定义的局部变量 a。因为在 block 块中使用到 a 局部变量，所以在 block 声明的时候这里才会将 a 作为参数传入，即 block 捕获了 a
   
        参数 4：flags 有默认值，也就说 flags 参数在调用的时候可以省略不传
   
                a(_a) 表示传入的 _a 参数会自动赋值给 a 成员，相当于 a = _a
      */
...
static void __main_block_func_0(struct __main_block_impl_0 *__cself) {
  int a = __cself->a; // bound by copy   // 创建了一个新的局部变量 a
    
    // a++;       // 实际上操作的是这里的局部变量 a，与开发者定义 block 时想要的不同，系统在编译阶段报错
    
    // 因为 block 在定义的之后已经将 a 的值传入存储在 __main_block_imp_0 结构体中，并在调用的时候将 a 从 block 中取出来使用，因此在 block 定义之后对局部变量进行改变是无法被 block 捕获的
...
static struct __main_block_desc_0 {
  size_t reserved;   // 0
  size_t Block_size; // sizeof(struct __main_block_impl_0)，存储着 __main_block_impl_0 的占用空间大小
