...
/**
  *  @brief   block 本质上也是一个 OC 对象，它内部也有一个 isa 指针。block 是封装了函数调用以及函数调用环境的 OC 对象。
  */
struct __block_impl {
  void *isa;     // 指向一个类对象，有三种类型：_NSConcreteStackBlock、_NSConcreteGlobalBlock、_NSConcreteMallocBlock。
  int Flags;     // block 的负载信息（引用计数和类型信息），按位存储
  int Reserved;  // 保留变量
  void *FuncPtr; // 一个指针，指向 Block 执行时调用的函数，也就是 Block 需要执行的代码块
};
...
/*   结构体：从命名上看是 main 中的第一个 Block（索引为 0） */
struct __main_block_impl_0 {
    
  struct __block_impl impl;
  struct __main_block_desc_0* Desc;
    
    // 同名构造函数 __main_block_imp_0，构造函数中对一些变量进行了赋值最终会返回一个结构体
  __main_block_impl_0(void *fp, struct __main_block_desc_0 *desc, int flags=0) {
    impl.isa = &_NSConcreteStackBlock; // 存储着 &_NSConcreteStackBlock 地址，可以理解为其类对象地址，block 就是 _NSConcreteStackBlock 类型的
    impl.Flags = flags;  // Block_byRef
    impl.FuncPtr = fp;   // Block 代码块中的代码被封装成 __main_block_func_0 函数，FuncPtr 则存储着 __main_block_func_0 函数的地址
    Desc = desc;         // 指向 __main_block_desc_0 结构体对象，其中存储 __main_block_impl_0 结构体所占用的内存
  }
};
...
int main() {
    ...
    // 精简（去掉强转）：
    // ①、定义 block 变量。调用了 __main_block_impl_0 函数，并且将 __main_block_impl_0 函数的地址赋值给了 block
...
//    void (*block)(void) = ((void (*)())&__main_block_impl_0((void *)__main_block_func_0, &__main_block_desc_0_DATA));
    ...
    /* ！！！疑问：block 指向的是 __main_block_impl_0 类型结构体，它并不直接就可以找到 FunPtr，因为 FunPtr 是存储在 __block_impl 中的，为什么 block 可以直接调用 __block_impl 中的 FunPtr 呢？
     
             查看源代码可以发现，(__block_impl *)block 将 block 强制转化为 __block_impl 类型的，因为 __block_impl 是 __main_block_impl_0 结构体的第一个成员，相当于将 __block_impl 结构体的成员直接拿出来放在 __main_block_impl_0 中，那么也就说明 __block_impl 的内存地址就是 __main_block_impl_0 结构体的内存地址开头，所以可以转化成功，并找到 FunPtr 成员。
           */
    block->FuncPtr(block);
